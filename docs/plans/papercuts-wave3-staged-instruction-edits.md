...
- `env.sh` initializes an unset or empty PATH to the deliberate Homebrew/local/system macOS base before sourcing cargo or adding planned local paths, and avoids empty PATH components. `agent.sh` removes every existing present gnubin occurrence before prepending the guarded coreutils, gnu-sed, gawk, findutils, grep list once in that order; repeated sourcing is idempotent. Name-only tests cover unset, empty, and nonempty PATHs; all gnubin paths late; exact prefix order; no empty components; GNU `sed`/`awk` under `agent.sh`; and BSD-first `sed`/`awk` with `env.sh` alone on a normal macOS PATH. No global instruction files were changed.
