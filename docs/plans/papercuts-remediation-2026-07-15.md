...
## Amendments (2026-07-16, post-r2)

- **Wave 6 ownership moved:** implemented by the delegate-agent session (branch `feature/wave6-hardening` in ~/Code/delegate-agent), not this repo's wave engine. Source: Trey-directed inter-agent mail 20260716-114726-8a3ef0.
- **Wave 6.1 amended by Trey ruling:** the work-mode worktree-default flip is overturned — work mode stays in-place by default (`isolation.work` config is the opt-in). Dirty trees on explicit worktree runs auto-apply `--include-dirty` with a loud pre-launch disclosure, no consent gate.
- **Wave 6 item 3 amended in implementation:** empty-result retry restricted to safe + read-only/pure calls (review caught a write-capable double-execution hole).
- **Wave 8 note:** the three delegate classification cuts (pc_c7f83ba034d1, pc_92319c59a164, pc_b967e7071e47) are resolved by the delegate-agent session, not this wave engine — do not double-resolve.
