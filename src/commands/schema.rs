...
pub fn contract(target: SchemaTarget) -> Value {
    ...
    match target {
        ...
        SchemaTarget::All => json!({
            ...
            "commands": {
                "add": {"alias":["log"],"positional":"TEXT or -; optional when stdin is piped","flags":{"--agent":"NAME","--tag":"TAG; repeatable","--severity":"minor|major|blocker; default minor","--cmd":"TEXT; optional evidence command","--exit":"N; optional evidence exit status","--stderr-file":"PATH; regular UTF-8 file up to 1 MiB, redacted then stored up to 4096 bytes; symlinks follow targets","--evidence":"TEXT; optional evidence note","--dry-run":"boolean"},"output":"{changed,record}","read_only":false,"appends":true,"destructive":false},
